...
| `LP-006` | When every fact is true and every control fired, distrust the THEREFORE — a behaviour claim owes an execution; reading gives the mechanism, only running gives the consequence | verification | high | 2026-07-28 |
| `LP-008` | When a gate is green over an auth/billing/deletion boundary, read the ASSERTION NAME as a claim — a non-vacuous guard aimed at the wrong proposition converts a defect into a requirement every future gate defends | testing | high | 2026-07-29 |<!-- EXAMPLE row (delete on the first real promotion):
